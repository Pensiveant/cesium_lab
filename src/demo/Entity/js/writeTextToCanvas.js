...
/**
 * 修改Cesium.writeTextToCanvas，使其支持文本垂直排列
 ...
 * @param {number} [options.column=false] 是否垂直排列
...
function writeTextToCanvas(text, options) {
  ...
  canvas.width = options?.column ? height : width;
  canvas.height = options?.column ? width : height;
